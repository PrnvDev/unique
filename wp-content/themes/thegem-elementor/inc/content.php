							...
							<div class="post-meta date-color">
								<div class="entry-meta clearfix">
									...
									<div class="post-meta-left">
										<span class="post-meta-date gem-post-date gem-date-color small-body"><?php echo get_the_date('d M Y'); ?></span>
									</div>
								</div><!-- .entry-meta -->
							</div>
						</div>
...
function thegem_build_icon($pack, $icon, $class = '') {
	if($icon) {
		if(in_array($pack, array('elegant', 'material', 'fontawesome', 'userpack', 'thegemdemo'))) {
...
function thegem_get_post_featured_content($post_id, $thumb_size = 'thegem-blog-default-large', $single = false, $picture_sources=array(), $audio_with_thumb = false) {
	...
	} else if (get_post_type($post_id) == 'post' || get_post_type($post_id) == 'page' || get_post_type($post_id) == 'product' || get_post_type($post_id) == 'thegem_news') {
...
function thegem_tribe_events_views_v2_bootstrap_html($html, $context, $view_slug, $query) {
	...
	$html_after .= '</div></div></div><!-- .block-content --></div>';
